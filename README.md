...
The site is server-rendered but has no database and no user accounts, which
leaves two honest options:

**Static.** Run `freeze.py` and deploy the `build/` folder. Works on Netlify,
Cloudflare Pages or GitHub Pages, free, with automatic HTTPS via Let's Encrypt.
You lose `/api/reference-code` — the page already falls back to generating the
code in the browser, which is fine for a prototype but means the server never
sees it.
